...
async function main() {
    ...
    const url = new URL(
        // "nike.splat",
        // location.href,
        params.get("url") || "http://127.0.0.1:5000/download/empty.splat",
    );
    ...
    function selectFileByUrl(fileUrl) {
        // Fetch the file
        fetch(fileUrl)
            .then(response => {
                if (!response.ok) {
            ...
            .then(blob => {
                // Call the selectFile function with the file blob
                selectFile(blob);
    ...
    // Fetch the file URL from the Flask API
    fetch('http://127.0.0.1:5000/get_3d_file_url')
    .then(response => {
        if (!response.ok) {
    ...
    .then(data => {
        ...
        // Call selectFileByUrl with the file URL
        selectFileByUrl(fileURL);
    })
    .catch(error => {
...
//fileURL = "http://127.0.0.1:5000/download/2024-06-04_18-47-10/output/1d55d614-3/point_cloud/iteration_3000/point_cloud.ply"
