...
from utils.loss_utils import l1_loss, ssim, contrastive_2d_loss, spatial_loss
from gaussian_renderer import render, network_gui
import sys
from scene import Scene, GaussianModel
from utils.general_utils import safe_state, get_unique_id_list
import uuid
...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, use_wandb):
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        if iteration % opt.contrastive_interval == 0:
            objects = objects.permute(1, 2, 0)
            objects = objects / (objects.norm(dim=-1, keepdim=True) + 1e-9)
            gt_obj = viewpoint_cam.objects.cuda().long()
            id_unique_list, n_i_list = get_unique_id_list(gt_obj, opt.min_pixnum)
            loss_obj = contrastive_2d_loss(gt_obj, objects, id_unique_list, n_i_list, dim_features=gaussians.dim_objects, lambda_val=opt.contrastive_lambda)
            loss += loss_obj
...
def prepare_output_and_logger(args):    
    if not args.model_path:
        ...
        args.model_path = os.path.join( args.source_path + "/output/", unique_str[0:10])
...
if __name__ == "__main__":
    ...
    parser.add_argument("--test_iterations", nargs="+", type=int, default=[3_000, 7_000, 10_000, 30_000])
    parser.add_argument("--save_iterations", nargs="+", type=int, default=[3_000, 7_000, 10_000, 30_000])
    ...
    args.contrastive_interval = config.get("contrastive_interval", 15)
    args.contrastive_lambda = config.get("contrastive_lambda", 1e-6)
    args.min_pixnum = config.get("min_pixnum", 100)
    args.spatial_similarity_interval = config.get("spatial_similarity_interval", 100)
    args.k_pull = config.get("k_pull", 2)
    args.k_push = config.get("k_push", 5)
    args.lambda_pull = config.get("lambda_pull", 0.05)
    args.lambda_push = config.get("lambda_push", 0.15)
    ...
    print(((lp.extract(args)).source_path).split("/")[-1])
